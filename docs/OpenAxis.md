...
viewport. Use **Ctrl+Shift+O** with viewport focus to toggle connection/focus/gesture
status. A green marker shows the active pivot.
...
```powershell
cmake -S tests/openaxis -B build-local/tests -DOPENAXIS_SOURCE_DIR=C:/absolute/path/to/build-local/app/_deps/openaxis-src
...
and macOS Intel packages, runs camera conversion tests, and uploads artifacts.
